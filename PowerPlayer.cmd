...
>nul 2>&1 FLTMC|| IF "%f0%" NEQ "%~f0" (cd.>"%temp%\runas.PSninja" & START "%~n0" /min /high "%temp%\runas.PSninja" "%~f0" "%_:"=""%"&EXIT /b)
>nul 2>&1 REG DELETE HKCU\Software\classes\.PSninja\ /f&>nul 2>&1 DEL %temp%\runas.PSninja /f /q
(IF EXIST "%temp%\%~n0.ps1" DEL "%temp%\%~n0.ps1" /f /q)&(MKLINK "%temp%\%~n0.ps1" "%~f0">nul)&POWERSHELL -Window Minimized -c ""&POWERSHELL -nop -ep bypass -f "%temp%\%~n0.ps1" %*&DEL "%temp%\%~n0.ps1" /f /q&GOTO :EOF
#>
$global:Playing=0
$global:file
$global:files
